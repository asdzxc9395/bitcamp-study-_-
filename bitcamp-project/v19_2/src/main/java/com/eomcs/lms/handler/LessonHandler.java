...
public class LessonHandler {
  ...
  public void addLesson() {
    ...
    lesson.setNo1(inputInt("번호? "));
    lesson.setTitle(inputString("수업명? "));
    lesson.setContents(inputString("설명? "));
    lesson.setStart(inputDate("시작일? "));
    lesson.setEnd(inputDate("종료일? "));
    lesson.setTotal(inputInt("총수업시간? "));
    lesson.setDay(inputInt("일수업시간? "));
  ...
  public void updateLesson() {
    ...
    newLesson.setContents(inputString("설명? ", oldLesson.getTitle()));
    
    newLesson.setStart(inputDate(
        String.format("시작일(%s)? ", oldLesson.getStart()), 
        oldLesson.getStart()));
    
    newLesson.setEnd(inputDate(
        String.format("종료일(%s)? ", oldLesson.getEnd()), 
        oldLesson.getEnd()));
    
    newLesson.setTotal(inputInt(
        String.format("총수업시간(%d)? ", oldLesson.getTotal()), 
        oldLesson.getTotal()));
    
    newLesson.setDay(inputInt(
