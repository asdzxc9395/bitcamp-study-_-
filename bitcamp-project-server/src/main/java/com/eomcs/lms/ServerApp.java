      ...
      while (true) {
        ...
        if (request.equals("/board/list")) {
          ...
          try {
            ...
            if (i == boards.size()) { // 같은 게시물의 번호가 없다면?
              boards.add(board); // 새 게시물을 등록한다.
              out.writeUTF("OK");
            } else {
              out.writeUTF("FAIL");
              System.out.println("같은 번호의 게시물이 있습니다.");
            }
          ...
          try {
            ...
            if (index != -1) { // 삭제하려는 게시물의 번호를 찾았다면
              boards.remove(index);
              out.writeUTF("OK");
          ...
          try {
            ...
            if (i == members.size()) { // 같은 게시물의 번호가 없다면?
              members.add(member);
              out.writeUTF("OK");
            } else {
          ...
          try {
            ...
            if (i == lessons.size()) { // 같은 게시물의 번호가 없다면?
              lessons.add(lesson);
              out.writeUTF("OK");
            } else {
