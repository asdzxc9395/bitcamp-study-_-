...
// 방법 2)
// => 방법 1처럼 기존 규칙처럼 새 규칙을 만들면 ,
// => 계속 규칙이 늘어나 관리하기가 어려운 문제가 있다.
//    그리고 규칙에 따라 구분하도록 코디해야 하기 때문에 프로그래밍 하기가 불편하다.
//    기존 규칙에 새 규칙을 추가한다면 이런 문제가 제거된다.
// => touch 기능을 기존 규칙(Computer2)에추가한다.
// => 문제는 touch()라는 새 규칙을 추가하기 되면
//    기존에 구현한 클래스는 모두오류가 발생할 것이다.
//    왜?
//    규칙이 달라졌기 떄문에 클래스 구현오류가 발생하는 것이다.
