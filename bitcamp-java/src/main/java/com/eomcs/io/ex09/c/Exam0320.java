...
public class Exam0320 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // 테스트 2:
    // Exam0310 에서 Member 객체를 출력한다.
    // Member 클래스에 인스턴스필드 'tel'를 추가한다.
    // toString()도 변경한다.
    // Exam0320에서 Member 객체를 읽는다.
    // 결과 : java.io.InvalidClassException 오류가 발생한다.
    // 이유 ?
    // deserialize 할 때,
    // 즉 readObject() 를 통해 바이트 배열을 읽어 객체를 생성할 때,
    // 같은 클래스인지 검사한다.
    // 만약 다른 클래스라면 InvalidClassException 예외가 발생한다.
    //
    // 검사방법?
    // serialize 할때 사용한 클래스와 deserialize 할 때
    // 사용한 클래스가 같은지 검사하는 방법?
    // (serialize 할 때 사용할 Member 클래스와
    // deserialize 할 때 사용할 Member 클래스가 동일한 클래스인지 검사하는 방법)
    // - 클래스 에 내장된 스태틱 변수인 serialVersionUID의 값을 비교한다.
    //
    // serialVersionUID
    ...
    // - Exam0310 Member 객체를 출력할 때 이 변수의 값도 출력하였다.
    // - Member 클래스를 변경하였다.
    // - 컴파일러 또한 이 클래스의 serialVersionUID 변수의 값을 변경하였다.
    // - Exam0320에서 바이트 배열을 읽어서 Member 객체를 생성하려 하였다.
    // - 그러나 파일이 저장된 Member 클래스의 serialVersionUID의 값과
    // 실제 객체를 생성하기 위해 사용할 Member클래스의 serialVersionUID의 의 값이
    // 달라서
    // JVM은 저장할때 형식과 읽을때 메모리 형식이 다르다고 판단하여 오류를 띄웠다.
